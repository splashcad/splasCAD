(() => {
  if(location.pathname.endsWith('/recovery-hob.html')){
    location.replace('/recovery-r004.html');
    return;
  }
  ...
  if(tabletChoosePhotoButton){
    ...
    const showPhotoLoading=()=>{tabletChoosePhotoButton.disabled=true;tabletChoosePhotoButton.textContent='Loading photo…';tabletChoosePhotoButton.classList.add('scan-working');};
    const finishPhotoLoading=()=>{tabletChoosePhotoButton.disabled=false;tabletChoosePhotoButton.textContent='Choose / change photo';tabletChoosePhotoButton.classList.remove('scan-working');};
    ...
    tabletTakePhotoButton.addEventListener('click',async()=>{
      ...
      overlay.querySelector('[data-camera-capture]').addEventListener('click',()=>{
        ...
        canvas.toBlob(blob=>{
          ...
          try{
            const dt=new DataTransfer();dt.items.add(file);cameraInput.files=dt.files;cameraInput.dispatchEvent(new Event('change',{bubbles:true}));
          }catch{
            const reader=new FileReader();reader.onload=()=>{const photo=document.getElementById('wallPhoto');if(photo)photo.src=String(reader.result||'');};reader.readAsDataURL(file);
          }
          close();
        },'image/jpeg',0.92);
      });
  ...
  if(tabletScanButton){
    tabletScanButton.addEventListener('click',()=>{
      ...
      const timer=setInterval(()=>{
        ...
        if((!working&&Date.now()-started>1500)||Date.now()-started>90000){clearInterval(timer);tabletScanButton.disabled=false;tabletScanButton.textContent=originalText;tabletScanButton.classList.remove('scan-working');}
      },250);
    });
  }
