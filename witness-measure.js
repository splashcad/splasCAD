(() => {
  const BUILD='ALPHA 6.0.22 · UPDATE 018';
  const start=()=>{
    ...
    const render=()=>{
      ...
      const runs=horizontalRuns();if(!runs.length)return;
      const left=Math.min(...runs.map(r=>r.x1))*overlay.width;
      const right=Math.max(...runs.map(r=>r.x2))*overlay.width;
      const glassBottom=Math.max(...runs.map(r=>r.y))*overlay.height;
      const datumY=Math.min(overlay.height-26*d,glassBottom+62*d);
      const red='#c55';
      c.strokeStyle=red;c.lineWidth=1.35*d;c.setLineDash([8*d,6*d]);
      // Bottom datum exists before any height is touched.
      line(c,left-26*d,datumY,right+26*d,datumY);
      // Witness drops from useful horizontal-line ends to the bottom datum.
      const xs=[];
      for(const r of runs){
        const y=r.y*overlay.height;
        if(y>=datumY-12*d)continue;
        for(const xN of [r.x1,r.x2]){
          const x=xN*overlay.width;
          if(!xs.some(v=>Math.abs(v-x)<18*d))xs.push(x);
        }
      }
      xs.sort((a,b)=>a-b);
      for(const x of xs){
        const candidates=runs.filter(r=>x>=r.x1*overlay.width-10*d&&x<=r.x2*overlay.width+10*d).map(r=>r.y*overlay.height).filter(y=>y<datumY-10*d);
        if(!candidates.length)continue;
        const startY=Math.max(...candidates);
        line(c,x,startY,x,datumY);
        line(c,x-7*d,datumY,x+7*d,datumY);
      }
      c.setLineDash([]);
    };
    const showHeightDatum=()=>{overlay.style.display='block';resize();};
    const hideHeightDatum=()=>{overlay.style.display='none';render();};
    heightButton?.addEventListener('click',()=>setTimeout(showHeightDatum,0));
    widthButton?.addEventListener('click',hideHeightDatum);
    enterButton?.addEventListener('click',hideHeightDatum);
    controls.querySelector('[data-edge-tool="clear"]')?.addEventListener('click',()=>setTimeout(()=>{if(heightButton?.classList.contains('active'))showHeightDatum();},0));

    new ResizeObserver(resize).observe(drawing);
    window.addEventListener('resize',resize);
    const drawingObserver=new MutationObserver(()=>{if(overlay.style.display!=='none')setTimeout(resize,0);});
    const status=document.getElementById('drawingStatus');if(status)drawingObserver.observe(status,{childList:true,characterData:true,subtree:true});
